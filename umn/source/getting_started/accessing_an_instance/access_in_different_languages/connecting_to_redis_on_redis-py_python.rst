...
-  To access a single-node, master/standby, read/write splitting, or Proxy Cluster instance, see :ref:`Accessing a Non-Redis Cluster Instance Using redis-py <dcs-ug-0312011__en-us_topic_0148195287_section63591955155713>`.
-  To access a Redis Cluster instance, see :ref:`Accessing a Redis Cluster Instance Using redis-py <dcs-ug-0312011__en-us_topic_0148195287_section14400336866>`.

.. _dcs-ug-0312011__en-us_topic_0148195287_section63591955155713:
...
#. Access the DCS Redis instance.
   ...
   a. If the ECS OS does not provide Python, run the following **yum** command to install it:
      ...
      #. Run the **ln -s python**\ *X.X.X* **python** command to create another Python link. In the command, *X.X.X* indicates the Python version number.

   b. Install Python and redis-py.
      ...
      #. Go to the directory where the decompressed redis-py package is saved, and install redis-py.
         ...
         .. figure:: /_static/images/en-us_image_0000002278891732.png
            :alt: **Figure 1** Running the python command

            **Figure 1** Running the python command

   c. Use the redis-py client to connect to the instance. In the following steps, commands are executed in CLI mode. (Alternatively, write the commands into a Python script and then execute the script.)

      #. Run the **python** command to enter the CLI mode. You have entered CLI mode if the following command output is displayed:


         .. figure:: /_static/images/en-us_image_0000002313524429.png
      ...
      #. Run the following command to access the chosen DCS Redis instance:
         ...
         *XXX.XXX.XXX.XXX* indicates the IP address/domain name of the DCS instance and **6379** is an example port number of the instance. For details about how to obtain the IP address/domain name and port, see :ref:`1 <dcs-ug-0312011__en-us_topic_0148195287_li450593110588>`. Change them as required. **\*****\*** indicates the password used for logging in to the chosen DCS Redis instance. This password is defined during DCS Redis instance creation. Omit the **, password='******'** part in the command for a password-free instance.
         ...
         .. figure:: /_static/images/en-us_image_0000002313491413.png
            :alt: **Figure 3** Redis connected successfully

            **Figure 3** Redis connected successfully

.. _dcs-ug-0312011__en-us_topic_0148195287_section14400336866:
...
#. Access the DCS Redis instance.
   ...
   b. Install the redis-py-cluster client.
   ...
   c. Access the DCS Redis instance by using redis-py-cluster.
      ...
      #. Run the following command to access the chosen DCS Redis instance:
         ...
         **\*****\*** indicates the password used for logging in to the chosen DCS Redis instance. This password is defined during DCS Redis instance creation. Omit the **, password='******'** part in the command for a password-free instance.
